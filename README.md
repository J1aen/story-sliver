> 🚀 **正式版 1.0 已上线**：http://101.35.148.131:8080
...
「匿名故事碎片墙」（Story Sliver）是一个轻量匿名 UGC 网站：注册用户发布一小段故事或生活碎片（发布时可选择匿名），提交后进入**待审核**状态，管理员通过后展示在「碎片墙」上，其他用户可以点赞。

- **核心体验**：温暖、克制的内容墙——米白纸感、瀑布流布局、手机优先适配
- **匿名设计**：匿名只影响对外展示，数据库始终记录真实发布者（`user_id`），方便管理员追溯违规内容
- **当前状态**：正式版 1.0 已上线，公网直连即可使用（IP + 8080 端口访问，无需备案）
...
- **为什么百度搜不到**：IP + 8080 端口的形式不会被搜索引擎收录；需要「域名 + 80 端口」，大陆服务器还需备案（香港服务器可免备案），详见 `fragment-story/deploy/部署指南.md`
