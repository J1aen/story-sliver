...
- **当前状态**：测试版 V1.2 已上线（IP + 8080 直连可用）；V2.0 评论功能前后端已完成、本地测试通过，待部署
...
| 评论（V2.0） | 登录用户可评论（免审核，发布即显示）；100 字上限、1 分钟 10 条；可删自己的评论（硬删除）、可举报（理由必填） |
| 公告（V1.2） | 站长发布公告，进站弹窗 + 顶部滚动条；同一标签页刷新不重弹，点 ✕ 才彻底消失 |
| 昵称与账号（V1.2） | 昵称唯一、修改昵称（敏感词即时拦截）；可修改密码（旧 token 失效） |
...
| 评论举报（V2.0） | 待处理举报列表；可不下架 / 下架评论 / 下架并封禁评论用户 |
| 公告管理（V1.2） | 站长在线编辑公告（标题/正文/图片/上下架） |
| 敏感词管理（V1.2） | 站长维护敏感词库，注册 / 改昵称即时拦截 |
...
│       └── styles/                     # 全局样式（温暖文字流风格）
├── 实现计划.md                         # 分阶段实现计划（Task 1-30 进度跟踪）
├── 制定计划.md                         # 产品设计与需求文档
...
### `sensitive_word`（敏感词库，V1.2）

| 字段 | 说明 |
| --- | --- |
| id / word / created_at | 主键 / 敏感词（唯一）/ 创建时间 |

用途：注册与修改昵称时即时拦截（站长可在管理后台维护）。

### `announcement`（公告，V1.2）

| 字段 | 说明 |
| --- | --- |
| id / title / content / image_url | 主键 / 标题（≤50 字）/ 正文（≤2000 字）/ 图片（可空） |
| status / created_at / updated_at | 0 下架 1 上架 / 创建时间 / 更新时间 |

用途：站长在线编辑公告，进站弹窗 + 顶部滚动条。

### `comment`（评论，V2.0）

| 字段 | 说明 |
| --- | --- |
| id / fragment_id / user_id | 主键 / 所属碎片 / 评论者 |
| content / status / created_at | 内容（≤100 字）/ 0 正常 1 已下架（当前用硬删除，字段预留）/ 创建时间 |

用途：登录用户对已发布碎片评论（免审核，发布即显示）。

### `comment_report`（评论举报，V2.0）

| 字段 | 说明 |
| --- | --- |
| id / comment_id / reporter_id | 主键 / 被举报评论 / 举报人（唯一约束防重复举报） |
| reason / status / handled_at | 举报理由（必填）/ 0 待处理 1 已处理 / 处理时间 |

用途：管理员处理举报（不下架 / 下架 / 下架并封禁评论用户）。
...
| PUT | /api/users/nickname | 修改昵称（唯一 + 敏感词校验，V1.2） |
| PUT | /api/users/password | 修改密码（旧 token 失效，V1.2） |

### 公告 `/api/announcements`（V1.2）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | /api/announcements/active | 当前上架公告（进站弹窗；游客可调） |

管理端（仅站长）：`/api/admin/announcements` 增删改查与上下架、上传公告图片。

### 评论 `/api/comments`（V2.0）

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | /api/comments | 发评论（免审核；1 分钟 10 条） |
| GET | /api/comments | 分页查某碎片评论（游客可看，登录标记 mine） |
| DELETE | /api/comments/{id} | 删自己的评论（硬删除） |
| POST | /api/comments/{id}/report | 举报（理由必填、防重复） |

管理端（管理员/站长）：`GET /api/admin/comments/reports` 待处理举报；`POST /api/admin/comments/reports/{id}/handle` 处理（dismiss / delete / ban）。
...
管理端补充（V1.2/V2.0）：公告 `/api/admin/announcements`（仅站长）、敏感词 `/api/admin/sensitive-words`（仅站长）、评论举报 `/api/admin/comments/reports`（管理员/站长）。
